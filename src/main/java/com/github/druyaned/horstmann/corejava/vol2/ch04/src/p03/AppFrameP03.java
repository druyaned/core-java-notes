package com.github.druyaned.horstmann.corejava.vol2.ch04.src.p03;
...
public class AppFrameP03 extends JFrame {
    ...
    private void setThreads() {
        ...
        interruptThread = new Thread(() -> {
            try (SocketChannel interruptible = SocketChannel.open(
                ...
                while (!interruptThread.isInterrupted()) {
                    ...
                    bufIn.position(0);
                    if (readCount == SocketTaskP03.LETTER_SIZE) {
        ...
        blockThread = new Thread(() -> {
            try (
                ...
                byte[] bytes = new byte[SocketTaskP03.LETTER_SIZE];
                int readCount;
