package com.github.druyaned.corejava.vol1.ch09.src.rbm;

import static com.github.druyaned.corejava.vol1.ch09.src.rbm.Color.BLACK;
import static com.github.druyaned.corejava.vol1.ch09.src.rbm.Color.RED;
...
public class RedBlackMap<K, V> {
    ...
    /**
     * Returns number of nodes of the map.
    ...
    /**
     * Returns comparator of key in the map.
     * @return comparator of key in the map
     */
    ...
    /**
     * Returns found {@link Node node} of the key in this map
     * or {@code null} if there is no such node. Complexity is O(log(n)).
     * 
     * @param key to found a node
     * @return found {@link Node node} of the key in this map
    ...
    public Node<K, V> get(K key) {
        ...
        for (Node<K, V> node = root; node != null; ) {
            if (comp.compare(node.key, key) == 0) {
                return node;
            } else if (comp.compare(node.key, key) < 0) {
                node = node.right;
            } else {
                node = node.left;
            }
    ...
    /**
     * Accepts the {@code action} for each {@link Node node} of the map in ascending order.
     * 
     * <P><i>Execution</i>:<ol>
