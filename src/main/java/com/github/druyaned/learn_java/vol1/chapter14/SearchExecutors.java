...
public class SearchExecutors extends SearchRecursive {
    ...
    public SearchExecutors(String keyword) {
        super(keyword);
...
class DirSearch implements Callable<Integer> {
    ...
    public DirSearch(File where, String keyword, LinkedList<File> keyFiles, ExecutorService pool) {
        this.where = where;
    ...
    public Integer call() throws InterruptedException,
        ...
        if (where.isDirectory()) {
            ...
            for (File dirFile : dirFiles) {
                DirSearch search = new DirSearch(dirFile, keyword, keyFiles, pool);
    ...
    private synchronized void addToKeyFiles(File file) { keyFiles.add(file); }
}
