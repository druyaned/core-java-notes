...
public class TestUser {
    ...
    public static void run() {
        ...
        EventQueue.invokeLater(() -> {
            ...
            Path dirPath = Volume1.getDataDirPath().resolve("chapter12").resolve("txt-files");
            Path file1Path = dirPath.resolve("file1.txt");
            Path file2Path = dirPath.resolve("file2.txt");
            try {
                if (!Files.exists(dirPath)) {
                    Files.createDirectories(dirPath);
                }
                String jarDir = "/vol1/chapter12/txt-files/";
                if (!Files.exists(file1Path)) {
                    Files.createFile(file1Path);
                    InputStream in = TestUser.class.getResourceAsStream(jarDir + "file1.txt");
                    Files.copy(in, file1Path, StandardCopyOption.REPLACE_EXISTING);
                }
                if (!Files.exists(file2Path)) {
                    Files.createFile(file2Path);
                    InputStream in = TestUser.class.getResourceAsStream(jarDir + "file2.txt");
                    Files.copy(in, file2Path, StandardCopyOption.REPLACE_EXISTING);
                }
            } catch (IOException exc) {
                throw new UncheckedIOException(exc);
            }
            
            fileChooser.setCurrentDirectory(dirPath.toFile());
