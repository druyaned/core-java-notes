package druyaned.corejava.vol2.ch04network.t03interruptible;
    ...
    public static void run(JTextArea interruptArea, JTextArea blockArea) {
        new Thread(() -> {
            try (ServerSocket server = new ServerSocket(8189)) {
                ...
                new Thread(new SocketTask(socket1, interruptArea)).start();
                new Thread(new SocketTask(socket2, blockArea)).start();
