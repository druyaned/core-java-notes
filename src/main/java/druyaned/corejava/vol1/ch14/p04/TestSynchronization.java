package druyaned.corejava.vol1.ch14.p04;
...
public class TestSynchronization implements Runnable {
    ...
    private static void threadLocal() {
        System.out.println("\n" + bold("ThreadLocal"));
        ThreadLocal<LocalDateTime> currTime = ThreadLocal.withInitial(LocalDateTime::now);
        final int THREAD_COUNT = 3;
        Thread[] threads = new Thread[THREAD_COUNT];
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < THREAD_COUNT; i++) {
            threads[i] = new Thread(() -> {
                LocalDateTime dateTime = currTime.get();
                synchronized (sb) {
                    sb.append("  dateTime=currTime.get()=")
                            .append(dateTime)
                            .append("; dateTime.hashCode()=")
                            .append(dateTime.hashCode())
                            .append("; thread=")
        ...
        System.out.println("Threads (amount=" + THREAD_COUNT + ") have completed their tasks.");
