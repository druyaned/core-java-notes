package druyaned.corejava.vol1.ch08generics.t02array;
...
public class GenericArray<T> {
    ...
    @SafeVarargs
    private GenericArray(T... elements) {
    ...
    @SafeVarargs
    public static <T> GenericArray<T> from(T... elements) {
        return new GenericArray<>(elements);
    }
    
    private static void checkCapacity(int capacity) {
        if (capacity < 0 || capacity > MAX_CAPACITY) {
            throw new IllegalStateException("capacity must be from "
