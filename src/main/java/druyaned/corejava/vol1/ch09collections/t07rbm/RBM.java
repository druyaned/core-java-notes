package druyaned.corejava.vol1.ch09collections.t07rbm;

/**
 * Red-Black Map implementation. That's a self-balancing binary search tree
 ...
 * are {@link #put insertion} and {@link #remove removal}.
 ...
 * 
 * @author druyaned
 * @param <K> the type of key maintained by the {@link Node node}
 * @param <V> the type of value maintained by the {@link Node node}
 * @see Node
 */
public class RBM<K, V> {
    
    private Node<K, V> root;
    private int size;
    private final java.util.Comparator<K> comp;
    
    /**
     * Creates a new Red-Black Map with the given comparator to compare keys.
     * @param comp to compare keys
     */
    public RBM(java.util.Comparator<K> comp) {
        this.comp = comp;
    }
    ...
    /**
     * Returns comparator for the keys in the tree.
     * @return comparator for the keys in the tree
     */
    ...
    /**
     * Returns found {@link Node node} of the key in this tree
     * or {@code null} if there is no such key. Complexity is O(log(n)).
     * 
     * @param key to found a node
     * @return found {@link Node node} of the key in this tree
     *      or {@code null} if there is no such key
     */
    public Node<K, V> getNode(K key) {
        for (Node<K, V> node = root; node != null; ) {
            int comparison = comp.compare(node.key, key);
            if (comparison < 0)
                node = node.right;
    ...
    /**
     * Returns the last node (highest) in the map; if the map is empty throws an exception.
     * @return last node in the map; if the map is empty throws an exception
     */
    ...
    /**
     * Returns the least node strictly greater than the given key,
     * or null if there is no such key.
     * 
     * @param key for strictly comparison
     * @return least node strictly greater than the given key,
     *      or null if there is no such key
     */
     ...
     */
    public void forEach(java.util.function.Consumer<Node<K, V>> action) {
        ...
        while (stackSize > 0) {
            Node<K, V> node = (Node<K, V>)stack[--stackSize];
            action.accept(node);
            node = node.right;
            while (node != null) {
                stack[stackSize++] = node;
    ...
    /**
     * Inserts a node with the key and value into the map.
     * 
     ...
     * </pre>
     * 
     * @param key node's key to be inserted
     * @param value node's value to be inserted
     * @return previous value of the key's node or {@code null} if there hasn't been any
     */
    public V put(K key, V value) {
        ...
        Node<K, V> node = root, parent;
        int comparison;
        do {
            parent = node;
            comparison = comp.compare(node.key, key);
            if (comparison < 0)
                node = node.right;
            else if (comparison > 0)
                node = node.left;
    ...
    private void insertionBalance(Node<K, V> node) {
        // cases 1, 2
        while (node.parent != null && node.parent.color == Color.RED) {
            // case 3: recolor G, P and U
            Node<K, V> u = node.uncle();
            if (u != null && u.color == Color.RED) {
            // && node.parent.color == Color.RED
                node.parent.color = Color.BLACK;
                u.color = Color.BLACK;
                Node<K, V> g = node.grandpa();
                g.color = Color.RED;
                node = g;
    ...
    /**
     * Removes a node of the key from this map, if there has been any.
     * See the illustration definitions in the {@link #put insertion}.
     * 
     ...
     * 
     * @param key to find the node for removal
     * @return value of the removed key's node or {@code null} if there hasn't been any
     */
    public V remove(K key) {
        Node<K, V> found = getNode(key);
        if (found == null)
            return null;
        V removedValue = found.value;
        Node<K, V> limit = found; // l==null && r==null
    ...
    private void deleteLimit(Node<K, V> limit) {
        if (limit.color == Color.RED) { // simple case 1
            replaceLimitParent(limit, null);
            return;
        } // limit.color == Color.BLACK
        Node<K, V> child = limit.left == null ? limit.right : limit.left;
    ...
    private void replaceLimitParent(Node<K, V> limit, Node<K, V> child) {
    ...
    private void removalBalance(Node<K, V> node) {
    ...
    /**
     * Color of the {@link Node node} of the {@link RBM Red-Black Map}.
    ...
    /**
     * The node is an actual component of the {@link RBM Red-Black Map}.
    ...
    public static class Node<K, V> {
        ...
        private static<K, V> Node<K, V> newRoot(K key, V value) {
            return new Node<>(key, value, Color.BLACK, null);
        }
        private static<K, V> Node<K, V> newNode(K key, V value, Node<K, V> parent) {
            return new Node<>(key, value, Color.RED, parent);
        }
        private static<K, V> boolean isBlack(Node<K, V> node) {
