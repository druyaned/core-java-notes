...
public class T03HeapSort extends Topic {
    
    private final Random RANDOM = new Random();
    
    public T03HeapSort(Chapter chapter) {
        super(chapter, 3);
    }
    
    @Override public String title() {
        return "Topic 03 HeapSort";
    }
    ...
    @Override public void run() {
        ...
        int size = generateInt(MIN_SIZE, MAX_SIZE);
        int minVal = -size;
        int maxVal = +size;
        int[] values = new int[size];
        for (int i = 0; i < size; i++)
            values[i] = generateInt(minVal, maxVal);
    ...
    private Object[] langSorting(int size, int[] values) {
    ...
    private Object[] heapSorting(int size, int[] values) {
        int[] sorted = new int[size];
        HeapInt heap = new HeapInt(size, Integer::compare);
        long t1 = System.currentTimeMillis();
        { // sorting starts
            for (int i = 0; i < size; i++) {
                heap.add(values[i]);
    ...
    private void checkArrays(int size, int[] langValues, int[] heapValues) {
        for (int i = 0; i < size; i++) {
