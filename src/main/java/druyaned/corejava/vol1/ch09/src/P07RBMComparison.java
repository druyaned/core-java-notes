...
import druyaned.corejava.vol1.ch09.src.rbm.RBM;
import druyaned.corejava.vol1.ch09.src.rbm.RBMIndexed;
import java.util.ArrayList;
import java.util.Random;
import java.util.TreeMap;

public class P07RBMComparison implements Runnable {
    ...
    private static void comparison(int testCount, int size, int minVal, int maxVal) {
    ...
    private static long additionTest(TreeMap<Integer, String> tree, int[] arr) {
    ...
    private static long additionTest(RBM<Integer, String> tree, int[] arr) {
        long start = System.currentTimeMillis();
        for (int num : arr)
            tree.put(num, SOME_STRING);
        long stop = System.currentTimeMillis();
    ...
    private static long additionTest(RBMIndexed tree, int[] arr) {
        long start = System.currentTimeMillis();
        for (int num : arr)
            tree.put(num, SOME_STRING);
        long stop = System.currentTimeMillis();
    ...
    private static long removalTest(TreeMap<Integer, String> tree, int[] arr) {
        long start = System.currentTimeMillis();
        for (int num : arr)
            tree.remove(num);
        long stop = System.currentTimeMillis();
    ...
    private static long removalTest(RBM<Integer, String> tree, int[] arr) {
        long start = System.currentTimeMillis();
        for (int num : arr)
            tree.remove(num);
        long stop = System.currentTimeMillis();
    ...
    private static long removalTest(RBMIndexed tree, int[] arr) {
        long start = System.currentTimeMillis();
        for (int num : arr)
            tree.remove(num);
        long stop = System.currentTimeMillis();
    ...
    private static int generateInt(int minVal, int maxVal) {
