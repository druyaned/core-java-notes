...
import druyaned.corejava.vol1.ch09.src.deque.Deque;
import java.io.PrintStream;

public class PrinterIndexed extends AbstractPrinter<RBTIndexed> {
    
    public PrinterIndexed(RBTIndexed target, String filler, PrintStream sout) {
    ...
    private int getTreeHeight() {
        ...
        deque.addLast(new NodeAndNumber(target.getRoot(), 1));
        while (!deque.isEmpty()) {
    ...
    private RBTIndexed.Node[] getTreeArray(int treeHeight) {
        ...
        Deque<NodeAndNumber> deque = new Deque<>(target.size());
        deque.addLast(new NodeAndNumber(target.getRoot(), 0));
        while (!deque.isEmpty()) {
            NodeAndNumber pair = deque.removeFirst();
            RBTIndexed.Node node = pair.node;
